...
if __name__ == "__main__":
    ...
    while iBande < len(Interest):
        ...
        #S'il y a des fichiers TIF intermediaires, la lecture est plus rapide
            ...
            logging.debug (NumBande,int(NumBande),ExtRay[0][0])

            del ds
            logging.debug (NumBande)
    ...
    if boolCloud==1:
        ...
        bb=0
        while bb<100:
            logging.info bb,np.percentile(DivB2, bb)
            bb=bb+10
        '''
        CouvNuage=np.mean(np.array(CloudMask60))
        logging.info ("cloud coverage {}".format(CouvNuage))
        fce.write("%s \n" %(CouvNuage)) #cvi : logging.info du % nuage sur l image traitee
        ...
        for Tj in range(0,NbTile):
            ...
            if np.count_nonzero(data[Bande11])/(data[Bande11].shape[0]*data[Bande11].shape[1]*1.0)>0.1:  #0.5 au debut
                ...
                while iBande < len(Interest):
                    ...
                    logging.info ("Ray %s : %s" %(NumBande,ExtRayleigh[iBande][0][0]))
                ...
                logging.debug("Temps Calcul1 %s seconds ---" % (time.time() - start_time))
                logging.info ("Haze: {}".format(Haze))
                Eps=[]
                iBande=0
                while iBande < len(Interest):
                    NumBande=int(Interest[iBande])
                ...
                logging.info ("{}\t{}\t{}\t{}".format(Ti,Tj,EpsMax,Haze[Bande11]))

                if Haze[Bande11]>0.02:
                    logging.info ("No Dark target %s-%s" %(Ti,Tj))
                ...
                if EpsMax>0:
                    EpsMax=0.0
                logging.debug("Temps Haze %s seconds ---" % (time.time() - start_time))
    ...
    if nbNonNan==0:
        logging.info ("No Dark target in the image --> QUIT")
        sys.exit()

    TotalMinB11=np.min(MinB11Array[MinB11Array>-1])
    logging.info ("{}".format(TotalMinB11))
    logging.info ("{}".format(EpsArray))
    ...
    logging.info ("---")
    logging.info ("{}".format(EpsArray))
    logging.info ("{}".format(MinB11Array))



    nbNonNan=NbTile*NbTile-np.count_nonzero(np.isnan(EpsArray))
    logging.info ("{}".format(nbNonNan))
    if nbNonNan==0:
        logging.info ("No Dark target in the image --> QUIT")
        sys.exit()
    '''
    ...
    logging.info ("{}".format(interpolatedEpsArray))

    logging.info ("{}".format(interpolatedMinB11Array))

    fce.write("%s \n" %(np.mean(interpolatedEpsArray))) #cvi : logging.info du coeff d angstrom calcule sur l'image
    fce.write("%s \n" %(np.mean(interpolatedMinB11Array))) #cvi : logging.info du coeff d angstrom calcule sur l'image
    fce.close
    ...
    while iBande < stepNumber:
            ...
            if dataToWrite == []:
                    ...
                    logging.debug ("Reading Bande %s" %Write[iBande])
                    band = ds.GetRasterBand(1)
                    SizeX = ds.RasterXSize
                    logging.debug("Temps Avant %s seconds ---" % (time.time() - start_time))
            ...
            tau_O3=getTau(Lamb[NumBande])
            logging.debug ("Ray %s : %s" %(NumBande,RayToWrite[0][0]))
        ...
        #Enregistrement des fichiers corriges - surface reflectances
            ...
            outDs.SetProjection(inDs.GetProjection())
            logging.info ("Bande %s SAVED" %Write[iBande])
            ...
            if int(NumBande)==4:
                    dataRef[2] = Correc
            logging.debug ("{}".format(NumBande))
            if(currentprogress>progress):
                progress=int(currentprogress)
                current_progress = (progress+2)
    ...
    logging.info ("Cloud SAVED")
    logging.info("Progress[%]: 93")
    try:
            ...
            logging.info ("Aerosol SAVED")
            logging.info("Progress[%]: 95")
            ...
            logging.info ("MinB11 SAVED")
            logging.info("Progress[%]: 99")
    except:
            logging.info ("Manque info mais pas grave")
